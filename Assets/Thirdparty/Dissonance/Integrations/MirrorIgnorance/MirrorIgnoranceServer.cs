...
namespace Dissonance.Integrations.MirrorIgnorance
{
        ...
        : BaseServer<MirrorIgnoranceServer, MirrorIgnoranceClient, MirrorConn>
    {
        ...
        {
            NetworkServer.RegisterHandler<DissonanceNetworkMessage>(OnMessageReceived);
        ...
        public override void Disconnect()
        {
            ...
            NetworkServer.RegisterHandler<DissonanceNetworkMessage>(MirrorIgnoranceCommsNetwork.NullMessageReceivedHandler);
        ...
        /// <summary>
        /// This MUST be called to inform Dissonance that a client has disconnected
        /// </summary>
        /// <param name="connection"></param>
        public static void ForceDisconnectClient(NetworkConnection connection)
        {
            if (_instance != null)
                _instance.ForceDisconnectClient(new MirrorConn(connection));
        }
        ...
        private void ForceDisconnectClient(MirrorConn conn)
        {
            ...
            if (index >= 0)
            {
                Log.Debug("Force disconnected Mirror connection '{0}'", conn);
        ...
        private bool Send(ArraySegment<byte> packet, MirrorConn connection, byte channel)
        {
            ...
            return connection.Connection.Send(new DissonanceNetworkMessage(packet), channel);
        }
        #endregion
    }
}
