...
namespace Dissonance.Integrations.MirrorIgnorance
{
        ...
        : BaseCommsNetwork<MirrorIgnoranceServer, MirrorIgnoranceClient, MirrorConn, Unit, Unit>
    {
        ...
        {
            NetworkServer.RegisterHandler<DissonanceNetworkMessage>(NullMessageReceivedHandler);
    ...
    internal struct DissonanceNetworkMessage
        : IMessageBase, IDisposable
    {
        private const int BufferLength = 1024;
        private static readonly ConcurrentPool<byte[]> SerializationBuffers = new ConcurrentPool<byte[]>(8, () => new byte[BufferLength]);
        ...
        {
            var arr = Data.Array;
            if (arr != null && arr.Length == BufferLength)
            {
                SerializationBuffers.Put(arr);
